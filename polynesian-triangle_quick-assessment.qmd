...
d |> 
  select(category, layer, dif_mu_sd, is_significant, avg_global, avg_polytri, dif_pval) |> 
  datatable() |>
  formatRound(columns=c(
    'dif_mu_sd', 'avg_global', 'avg_polytri', 'dif_pval'), digits=3)
```

This table describes overall results ordered by `category`, `dif_mu_sd` and `is_significant`. The `dif_mu_sd` represents the difference between averages ($\mu_{polynesia}$ - $\mu_{global}$) divided by the Global standard deviation ($sd_{global}$).
