...
r_sum <- sum(s_features, na.rm = T) %>% 
  mask(r_pu_id)
s_features <- stack(s_features, r_sum)
...
for (lyr in names(S)[27:length(names(S))]){ # lyr = names(S)[1]
  
  # setup paths to outputs
  r_tif <- glue("{dir_out}/{lyr}.tif")
  ...
  d_stats <- tibble(
    ...
    avg_global  = mu_g,
    avg_polytri = mu_p,
    pval_isdiff = w$p.value)
  
  if (!file.exists(stats_csv)){
    write_csv(d_stats, stats_csv)
  } else {
    read_csv(stats_csv, show_col_types = F) |> 
      filter(layer != lyr) |> 
      bind_rows(
        d_stats) |> 
      write_csv(stats_csv)
...
# [Preliminary Testing: The devil of statistics?](https://reinventionjournal.org/index.php/reinvention/article/download/339/415?inline=1#:~:text=Two%2Dstep%20test%20procedure%20with%20both%20equal%20variance%20and%20normality%20preliminary%20tests)
...
::: {.callout-note  collapse="true"} Statistical Aside

In order to answer the question, are the values significantly different, ie larger or smaller, than the Global, I used a Welchs' t-test, which does not assume equal variance between groups but does assume normal distribution. There are some other approaches to solving this.
